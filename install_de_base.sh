...
if ! grep -F -q 'Option "XkbLayout"' /etc/X11/xorg.conf.d/*; then
...
if ! [ -r /etc/skel/.xinitrc ] ||
  ! grep -F -q 'for f in $HOME/.xinitrc.d/*.sh; do' /etc/skel/.xinitrc; then
  echo '#!/bin/sh
...
. /etc/X11/xinit/xinitrc  # Sourcing system-wide xinitrc.' >.xinitrc
  chmod o+rx .xinitrc
...
if ! [ -r /etc/shrc ] || ! grep -F -q 'for f in /etc/shrc.d/*.sh; do' /etc/shrc; then
  echo '#!/bin/sh
# /etc/shrc
...
if ! [ -r /etc/skel/.shrc ] ||
  ! grep -F -q 'for f in $HOME/.shrc.d/*.sh; do' /etc/skel/.shrc; then
  echo '#!/bin/sh
...
if ! [ -r /etc/skel/.profile ] ||
  ! grep -F -q 'for f in $HOME/.profile.d/*.sh; do' /etc/skel/.profile; then
  echo '#!/bin/sh

# The recommended way to configure the POSIX login shell startup is to place
# your readable configs in $HOME/.profile.d/*.sh .

export ENV=$HOME/.shrc  # The recommended POSIX shell interactive startup file.
